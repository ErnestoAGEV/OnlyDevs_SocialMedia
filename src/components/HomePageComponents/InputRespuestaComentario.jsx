...
import { useInsertarRespuestasComentariosMutate } from "../../stack/RespuestasComentariosStack";
...
export const InputRespuestaComentario = () => {
  ...
  const { mutate: comentarioMutate } = useInsertarRespuestasComentariosMutate({
    respuesta: comentario,
    setRespuesta: setComentario
  });
    ...
    <section className="flex items-center gap-2 p-4 bg-white dark:bg-neutral-900">
      <section className="w-full gap-2 flex flex-col">
        ...
        <section className="flex justify-end">
          <button
            className={`flex justify-end gap-1 px-4 py-2 rounded-full text-sm text-gray-500 cursor-not-allowed ${comentario.trim() === ""?"cursor-not-allowed text-gray-500":"cursor-pointer  text-[#00AEF0] hover:bg-blue-600/10"}`}
